...
const ReturnPage = () => {
    ...
    const handleReturn = async () => {
        try {
            const response = await axiosSecure.patch(`/api/bookings/update-return-status/${trackingId}`, {
                returnReason: reason,
                refundType: refundType
            });
            console.log(response.data);
            if (response.data.modifiedCount > 0) {
                toast.success("Your product has been returned successfully");
            }
            else{
    ...
    return (
        <div>
            <h1 className="text-4xl font-bold mb-4 text-amber-500 text-center pt-14">Return Product</h1>
            <p className='text-lg text-center text-gray-400'>Fill up the following form to return your product</p>
            <div className='flex justify-around items-center max-w-5xl mx-auto px-12 gap-12 mt-16'>
                ...
                <div className="w-1/2 mx-auto p-12 bg-white rounded shadow-lg my-16 border-2 border-amber-400">
                    ...
                    <div className="mb-4">
                        ...
                        <select id="reason" value={reason} onChange={(e) => setReason(e.target.value)} className="mt-1 w-full border-amber-500 rounded-md shadow-md focus:ring-amber-500 focus:border-amber-500 p-2 bg-amber-100 text-gray-400 font-medium">
                    ...
                    <div className="mb-4">
                        ...
                        <input type="text" id="trackingId" value={trackingId} onChange={(e) => setTrackingId(e.target.value)} className="mt-1 block w-full border-amber-300 rounded-md shadow-md focus:ring-amber-500 focus:border-amber-500 p-2 pl-4 bg-amber-100 text-gray-400 font-medium" placeholder='Tracking ID' />
                    </div>
                    <div className="mb-4">
                    ...
                    <button onClick={handleReturn} className="btn text-lg w-full bg-amber-400 text-white py-2 px-4 rounded hover:bg-amber-500">Return</button>
                </div>
                <div className='w-1/2 '>
